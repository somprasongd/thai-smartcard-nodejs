...
class PersonalApplet {
  ...
  async getInfo() {
    ...
    info.issuer = legacy.decode(data, 'tis620').slice(0, -2).toString().trim();
    // Issue Date
    ...
    data = legacy.decode(data, 'tis620');
    data = data.slice(0, -2).toString().trim().split('#');
    info.address = {
      houseNo: data[0],
      street: data.slice(1, -3).join(' ').trim(),
    ...
    let photo = hex64.toBase64(data.slice(0, -2).toString('hex').trim());
    data = await reader.getData(this.card, apduPerson.CMD_PHOTO2, this.req);
    photo += hex64.toBase64(data.slice(0, -2).toString('hex').trim());
